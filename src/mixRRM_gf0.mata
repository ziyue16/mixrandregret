	...
	for(n=1; n <= N_subject; n++) {
        ...
        for(t=1; t<=nc; t++) {
			...
			for(rr=1; rr<=nrep; rr++) {
					...
					// Create regret for each alternative 	
					/*ALVARO: Here I generate the row sum (I collapse the total regret of alternative i)*/
					ER_rep[ .,rr]  =  rowsum(regret_draw_r)	
				}
	
			ER = exp(-ER_rep) /*ALVARO: Here it is correct, we need the negative of the regret*/
			ER = (ER :/ colsum(ER,1)) /**/
